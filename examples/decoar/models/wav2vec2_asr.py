...
class Wav2Vec2AsrConfig(FairseqDataclass):
    ...
    dropout: float = field(
        default=0.1, metadata={"help": "dropout probability inside wav2vec 2.0 model"}
    )
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, tgt_dict=None):
        ...
        self.lstm = nn.LSTM(input_size=d, hidden_size=1024, num_layers=2,
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        if self.mask and self.training:
            x = self.apply_mask(x, padding_mask.unsqueeze(2).expand(x.shape))

        max_seq_len = x.shape[1]
        seq_lengths = (~padding_mask).sum(dim=-1)
        packed_rnn_inputs = pack_padded_sequence(x, seq_lengths.tolist(),
                                                 batch_first=True,
                                                 enforce_sorted=False)

        packed_rnn_outputs, _ = self.lstm(packed_rnn_inputs)
        x, _ = pad_packed_sequence(packed_rnn_outputs,
                                   batch_first=True,
