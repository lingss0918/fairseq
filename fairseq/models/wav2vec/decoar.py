...
class DeCoArModel(BaseFairseqModel):
    ...
    def __init__(self, args):
        ...
        if self.quantizer:
            ...
            #self.project_q = nn.Linear(vq_dim, args.encoder_embed_dim)

        self.mask_emb = nn.Parameter(
            torch.FloatTensor(80).uniform_()
        )

        self.encoder = TransformerEncoder(args)
        '''
        self.recon = TransformerSentenceEncoderLayer(
    ...
    def forward(self, source, padding_mask=None, features_only=False):

        features = source.clone()
        features, mask_indices = self.apply_mask(features, padding_mask)
        ...
        '''
        x = x.transpose(0, 1)
        x, z = self.recon(x, self_attn_padding_mask=None, need_weights=False)
        x = x.transpose(0, 1)
        '''
        ...
        result = {"x": x, "y": y, "padding_mask": padding_mask}
...
@register_model_architecture("decoar", "decoar")
